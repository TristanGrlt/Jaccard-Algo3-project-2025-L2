...
//    associé à t dont la valeur est égale à celle de l'objet pointé par ref au
//    sens de la fonction de comparaison. Renvoie ce numéro, ce rang si une
//    telle valeur existe. Renvoie sinon le numéro, le rang qu'aurait le nœud si
//    la référence ref était ajoutée à l'arbre.
...
//    à t en profondeur, de manière infixe, de la gauche vers la droite ou de la
//    droite vers la gauche selon que la valeur de dir est inférieure ou égale à
//    zéro ou non. Pour chaque sous-arbre non vide p, exécute fun_pre(context)
//    avant de parcourir le premier sous-arbre de p, puis fun(context, ref) sur
//    la référence stockée à la racine de p, puis fun_post(context) après avoir
//    parcouru le second sous-arbre de p. Si l'une des fonctions fun_pre, fun ou
//    fun_post renvoie une valeur non nulle, l'exécution prend fin et la
//    fonction bst_dft_infix_left_apply_context renvoie cette valeur. Sinon, la
//    fonction renvoie zéro. Il est permis que fun_pre et fun_post aient comme
//    valeur un pointeur nul ; aucune des exécutions correspondantes n'a lieu
//    dans ce cas.
