...
Nous avons rencontré un problème sur le calcule de l'appartenance de chaque mot à son/ses fichiers. (\textit{Explication du calcule durant l'ajout de l'intersection de chaque mots et problème de pointeur et pas nombre puis résolution. (Peut être cour exemple pour expliqué la détection d'un certain bit à 1 ou 0)}).

\begin{itemize}
    \item \code{jaccard_init()} Initialise un nouveau calcule de Jaccard pour le nombre de fichier et noms passé en paramètre.
\end{itemize}

\begin{itemize}
    \item \code{jaccard_add()} traite chaque mot lu dans un fichier, et met à jour à la fois la table de hachage, les cardinaux des fichiers, et le tableau des intersections

    \begin{itemize}
        \item {Cas 1 : Mot absent de la table de hachage} 
        \begin{itemize}
            \item Le mot est ajouté à la table avec une valeur \code{uint64_t} initialisée à \code{1ULL << file_index} 
            \item Le mot est également ajouté au holdall \code{j->hd} 
            \item Le cardinal du fichier courant (\code{j->cardinals[file_index]}) est incrémenté.
        \end{itemize}

        \item {Cas 2 : Mot déjà présent} 
        \begin{itemize}
            \item L'ancienne valeur \code{old_mask} est récupérée depuis la table.
            \item Une nouvelle valeur est calculée : \code{new_mask = old_mask | (1ULL << file_index)}.
            \item Si le fichier courant est nouveau (i.e., \code{new_mask != old_mask}), pour chaque fichier \code{i} déjà présent dans \code{old_mask}, on incrémente l'intersection entre \code{i} et \code{file_index} dans le tableau \code{j->inter}.
            Le cardinal du fichier courant est mis à jour.
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \code{jcrd_print_graph()} (\textit{ToDo : Tristan})
...
\begin{itemize}
\item Intersection : Récupérée via \code{j->inter[idx]}
\item Union : Calcule \[
\text{union}(i,j) = \text{cardinal}_i + \text{cardinal}_j - \text{intersection}(i,j)
\]
\item Distance : Applique la formule :
$distance = 1 - \frac{intersection}{union}$.\end{itemize}
\begin{itemize}
\item Les distances sont arrondies à 4 décimales
\item L'entrée standard (\code{STDIN}) est représentée par \code{""}
\item Les paires sont triées par ordre croissant d'indices
\end{itemize}
\end{itemize}
