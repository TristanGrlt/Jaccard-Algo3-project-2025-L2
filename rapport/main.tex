...
\begin{itemize}
    ...
    \item (\textit{Explication complète de l'intersection (Maysaloon). Le principe du calcule de la taille et de chaque case liès à qu'elle intersection mais pas le systheme de byte derrière })
...
\begin{itemize}
    ...
    \item \code{jaccard_add()} Ajoute le mots liès à l'indice du fichier passé en paramètre. (\textit{Explication lors de l'ajout d'un nouveau mots ou mot déja trouvé avec mise à jour de l'intersection et du cardinal}).
    \item \code{jcrd_print_graph()} (\textit{ToDo : Tristan})
    \item \code{jcrd_print_distance()} (\textit{ToDo : Maysaloon})
\end{itemize}
